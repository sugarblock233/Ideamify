...
- 图上直接新建（新增）：**+ 一级路线** / **+ 子节点** 在父节点旁打开虚线草稿卡，保存前不产生任何记录；保存失败（断网/版本冲突）草稿保留，重试不会重复创建。
- Markdown 表格与受管图片（新增）：正文可插入表格；插入或粘贴图片即上传为受管附件，阅读视图内联渲染并可点击放大。图片只经认证接口加载（令牌永不出现在图片 URL），外链图片一律不加载、降为占位条。
- 科研版本与泳道（新增）：给方案打 v1/v2/v3 阶段标签（与「记录 #N」保存编号无关），按版本筛选时路线祖先以上下文形式保留；泳道视图将版本 × 一级线路排成一张进度表，一眼看清每轮方案覆盖的路线。
...
默认 `RESEARCHMAP_AUTH_MODE=local`：本机单用户，无需账号或令牌。Compose 只绑定 `127.0.0.1`，本机模式请保留此绑定。需要远程访问时，显式改为 `RESEARCHMAP_AUTH_MODE=token`，在 `RESEARCHMAP_TOKENS` 中配置长随机具名令牌，并通过受保护网络或 HTTPS 代理访问。本机模式也可为 AI 配置具名令牌，以区分提交身份。
...
- **离开前保存。** 节点编辑需要手动保存。本机模式刷新后直接恢复当前地图，无需登录；返回管理页前，未保存草稿会提醒确认。
