...
10. **远端状态**：本地可核对的唯一远端事实是 `origin/main` 停在 `cab94b2`（v0.1 初版）——
    该提交**不含 `.github/`**，因此远端**从未执行过任何 workflow**（这也印证 G05 的
    "CI 未运行"不是保守措辞而是实况）。除此以外，topics / description / ruleset / 可见性 /
    tag / 依赖告警等**没有任何实际读取证据**，一律标"未验证/待所有者操作"，不写成"已配置"。
...
| 5 | push / tag / 发布 / 可见性 | **待所有者操作** | 本轮**未 push、未打 tag、未发布、未改可见性**；`origin/main` 仍停在 `cab94b2`（v0.1，无 `.github/`，故远端从未跑过 CI）；首次发布文案在 `CHANGELOG.md` Unreleased |
