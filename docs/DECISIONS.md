...
## 本机项目管理页与可选令牌模式（2026-09-15）

- 用户明确要求同一设备、同一使用者管理多个项目，且不需要登录验证。
  默认 `RESEARCHMAP_AUTH_MODE=local`，根路径进入项目管理；地图保留深链接、
  原有编辑/提交语义及离开草稿提醒。无需建立账号或持久登录体系。
- 本机模式只适用于回环监听（Compose 继续绑定 `127.0.0.1`）；Host/Origin/
  写入自定义头检查防止 DNS rebinding 和来自其他网页的操作。这些检查不能
  替代网络认证，不得在公开监听端口上使用本机模式。
- 本机未带令牌的作者为 `researcher`；配置并提供有效具名令牌时沿用令牌作者。
  显式无效令牌不回落到本机身份。CLI 无需本机令牌，仍只调用 HTTP API。
- 远程模式显式 opt-in 为 `token`，令牌配置仍必需。原有内存登录与具名令牌
  协议保留；默认模式变化是用户要求的产品决策，替代原 SPEC §9 的强制令牌。
- 此次不新增 Cookie、浏览器认证存储、会话表或数据库迁移。

## 科研版图的内容与阅读设计（2026-09-16，仅文档）

- 研究者反馈：地图难以从负责人角度理解，代码记录缺少研究意义，浅层浏览难以形成连续思路。新增 [改造交付书](RESEARCH_MAP_REDESIGN.zh-CN.md)，将研究总览、路线阅读与证据详情作为待实施方案，并给出阶段和验收场景；本条不宣称界面或接口已经改变。
- [研究内容编写指南](RESEARCH_AUTHORING.zh-CN.md) 现在即可用：先组织大路线，再拆当前子问题，用 rationale/decision 和明确关系说明研究承接，区分代码实现、实际执行与有证据的判断。同步中英文入口和 AI 交接指令。
- 设计继续使用主树表达归属，关系表达明确的研究陈述，不把父子边自动解释为因果。研究深度来自问题与证据的推进，不由层级数量衡量。
- 提案优先复用现有 Node/Relation 和写入协议；新阅读状态属于浏览器导航。候选 context/relations 增量为后续任务，当前 SPEC 与 API 合同仍保持。本批没有修改应用代码、数据库或实际研究内容。
