...
也不需要有模型 API Key——你只需要**一个访问令牌**和 CLI。所有写入都落在与
...
| `RESEARCHMAP_TOKEN` | 所有 `/api/*` 命令必需 | Bearer 访问令牌。**只走环境变量**：按合同，令牌不出现在命令行参数、文件、URL 或日志中。CLI 刻意不提供 `--token` 参数。 |
...
v0.1 **没有** `FORBIDDEN`/403 项目级门禁（单一受信任令牌空间，§1.1）。
